...
export async function exportLessonPlansDocx(
  ...
  sortedPlans.forEach((plan, planIdx) => {
    ...
    docChildren.push(
      new Paragraph({
        spacing: { before: 40, after: 20 },
        children: [new TextRun({ text: isEnPlan ? "I. OBJECTIVES" : "I. YÊU CẦU CẦN ĐẠT", bold: true, color: "0F172A", font, size: baseSize })],
      })
    );
    ...
    docChildren.push(
      new Paragraph({
        spacing: { before: 40, after: 20 },
        children: [new TextRun({ text: isEnPlan ? "II. TEACHING AIDS AND LEARNING MATERIALS" : "II. ĐỒ DÙNG DẠY HỌC", bold: true, color: "0F172A", font, size: baseSize })],
      })
    );
    docChildren.push(
      new Paragraph({
    ...
    docChildren.push(
      new Paragraph({
        children: [new TextRun({ text: isEnPlan ? "III. TEACHING AND LEARNING PROCEDURES" : "III. CÁC HOẠT ĐỘNG DẠY HỌC CHỦ YẾU", bold: true, color: "0F172A", font, size: baseSize })],
      })
    );
      ...
      new TableRow({
        ...
        children: [
          new TableCell({
            ...
            shading: { fill: "1E3A8A" },
            children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: isEnPlan ? "TEACHER'S ACTIVITIES" : "HOẠT ĐỘNG CỦA GIÁO VIÊN", bold: true, color: "FFFFFF", font, size: baseSize })] })],
          }),
          new TableCell({
            width: { size: colHalfWidth, type: WidthType.DXA },
            shading: { fill: "1E3A8A" },
            children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: isEnPlan ? "STUDENTS' ACTIVITIES" : "HOẠT ĐỘNG CỦA HỌC SINH", bold: true, color: "FFFFFF", font, size: baseSize })] })],
          }),
        ],
      }),
    ];
    ...
    docChildren.push(
      new Paragraph({
        children: [new TextRun({ text: isEnPlan ? "IV. POST-LESSON REFLECTION / NOTES" : "IV. ĐIỀU CHỈNH SAU BÀI DẠY", bold: true, color: "0F172A", font, size: baseSize })],
      })
    );
    docChildren.push(
      new Paragraph({
  ...
  sortedComboPlans.forEach((plan) => {
    ...
    const activityRows: TableRow[] = [
      new TableRow({
        tableHeader: true,
        children: [
          new TableCell({ width: { size: colHalfWidth, type: WidthType.DXA }, shading: { fill: "1E3A8A" }, children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: isEnPlan ? "TEACHER'S ACTIVITIES" : "HOẠT ĐỘNG CỦA GIÁO VIÊN", bold: true, color: "FFFFFF", font, size: baseSize })] })] }),
          new TableCell({ width: { size: colHalfWidth, type: WidthType.DXA }, shading: { fill: "1E3A8A" }, children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: isEnPlan ? "STUDENTS' ACTIVITIES" : "HOẠT ĐỘNG CỦA HỌC SINH", bold: true, color: "FFFFFF", font, size: baseSize })] })] }),
        ],
      }),
    ];
          ...
          children: [
            new TableCell({
              ...
              children: [
                ...
                new Paragraph({ children: [new TextRun({ text: isEnPlan ? `- Procedure: ` : `- Cách tiến hành: `, bold: true, font, size: baseSize }), new TextRun({ text: act.teacherActivity, font, size: baseSize })] }),
              ],
            }),
            new TableCell({
              width: { size: colHalfWidth, type: WidthType.DXA },
...
export async function buildWeeklyKHBDWithLBGFirstPageDocxBlob(
  ...
  weekDays.forEach((day, dayIndex) => {
    ...
    plansForDay.forEach((plan, planIdxInDay) => {
      ...
      const subSubjectPart = plan.subSubject ? ` (${plan.subSubject.toUpperCase()})` : "";
      const bannerText = isEnPlan
        ? `SUBJECT: ENGLISH ${plan.grade} - PERIOD: ${plan.curriculumPeriod || 1}${classTag}`
        : `MÔN: ${plan.subject.toUpperCase()}${subSubjectPart} - TIẾT PPCT: ${plan.curriculumPeriod || 1}${classTag}`;
      ...
      docChildren.push(
        new Paragraph({
          children: [
            new TextRun({ text: isEnPlan ? "IV. POST-LESSON REFLECTION / NOTES: " : "IV. ĐIỀU CHỈNH SAU BÀI DẠY: ", bold: true, font, size: baseSize }),
